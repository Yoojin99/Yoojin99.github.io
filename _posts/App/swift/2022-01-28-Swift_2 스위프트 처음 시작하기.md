...
메모나 리마인더의 용도로 실행되지 않을 텍스트를 포함시키는데 주석을 사용한다. Swift 컴파일러는 컴파일 할 때 주석을 무시한다.

Swift의 주석은 C의 주석과 매우 비슷하다. 한 줄 주석은 슬래시 두 개를 사용한다.

```swift
// This is a comment.
```

여러 줄 주석은 슬래시와 asterisk로 시작해서 asterisk와 슬래시를 붙여 끝낸다.
...
C의 주석과는 다르게, Swift의 여러 줄 주석은 다른 여러 줄 주석 안에 중첩될 수 있다. 이는 코드가 이미 여러 줄 주석이 있다 해도 넓은 영역의 코드를 주석 처리하기 쉽게 해준다.
